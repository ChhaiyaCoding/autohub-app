...
- **Firestore** — `providers` (public catalog, seeded once via `seed.html`) and `users/{uid}` (favorites field, plus `vehicles`/`bookings`/`notifications` subcollections, created automatically on first sign-in). Notifications are generated by real events — a confirmed booking, a dispatcher updating progress or replying in chat from `admin.html` — and stream to the app live
