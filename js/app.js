...
const App = {
  ...
  init() {
    ...
    // restore saved locations if any (notifications now live in Firestore)
    try {
  ...
  _watchAuthState() {
    ...
    window.authFns.onAuthStateChanged(window.auth, (user) => {
      if (user) {
        ...
        this._initNotifListener(user.uid);
        if (this.AUTH_SCREENS.includes(this.state.route)) { this.go('home'); this._history = []; }
        else this.render();
      } else {
        this.state.user = null;
        if (this._notifUnsub) { this._notifUnsub(); this._notifUnsub = null; }
  ...
  _userDataLoading: null,
  async _loadUserData(uid) {
    if (!window.db || !window.fs) return;
    // onAuthStateChanged can fire twice on a fresh sign-up; without this guard
    // both calls see "no user doc yet" and seed twice (duplicate welcome notif).
    if (this._userDataLoading === uid) return;
    this._userDataLoading = uid;
    try {
      ...
      if (!userSnap.exists()) {
        ...
        this._pushNotif(uid, { kind: 'welcome' });
      } else {
  ...
  // ---- Notifications (users/{uid}/notifications, generated by real events) ----
  // Written by: confirmBooking() (customer), and admin.html when a dispatcher
  // updates progress or replies in chat. Read live via onSnapshot.
  _notifUnsub: null,
  _pushNotif(uid, data) {
    if (!uid || !window.db || !window.fs) return;
    window.fs.addDoc(window.fs.collection(window.db, 'users', uid, 'notifications'), {
      ...data, read: false, createdAt: Date.now(),
    }).catch((e) => console.warn('Failed to write notification', e));
  },
  _initNotifListener(uid) {
    if (this._notifUnsub) { this._notifUnsub(); this._notifUnsub = null; }
    if (!uid || !window.db || !window.fs) return;
    const q = window.fs.query(
      window.fs.collection(window.db, 'users', uid, 'notifications'),
      window.fs.orderBy('createdAt', 'desc'));
    this._notifUnsub = window.fs.onSnapshot(q, (snap) => {
      this.state.notifications = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
      // Only re-render screens that display notifications — a full render
      // elsewhere would wipe whatever the user is typing in a form.
      if (['home', 'notifications', 'profile'].includes(this.state.route)) this.render();
    }, (e) => console.warn('Notifications listener failed', e));
  },
  ...
  confirmBooking() {
    ...
    this._pushNotif(this._currentUid(), {
      kind: 'booked', bookingId: booking.id, providerName: p ? p.name : '', service: booking.service, date: booking.date,
    });
    ...
    return `<div class="screen">
      <div class="home-header">
        ...
        <div style="display:flex;gap:10px;align-items:center">
          ...
          <div class="icon-btn" style="position:relative" onclick="App.go('notifications')">${icon('bell')}${this.state.notificationsEnabled && this.unreadNotifCount() ? '<span class="badge-dot"></span>' : ''}</div>
  ...
  _setNotifRead(id) {
    const uid = this._currentUid();
    if (!uid || !window.db || !window.fs) return;
    window.fs.setDoc(window.fs.doc(window.db, 'users', uid, 'notifications', id), { read: true }, { merge: true })
      .catch((e) => console.warn('Failed to mark notification read', e));
  },
  markAllNotifRead() {
    this.state.notifications.forEach((n) => { if (!n.read) { n.read = true; this._setNotifRead(n.id); } });
    this.render();
  },
  markNotifRead(id) {
    const n = this.state.notifications.find(x => x.id === id);
    if (n && !n.read) { n.read = true; this._setNotifRead(id); this.render(); }
  },
  ...
  // Fill {placeholders} in a translated string from a params object.
  _fmt(key, params = {}) {
    return t(key).replace(/\{(\w+)\}/g, (_, k) => (params[k] != null ? params[k] : ''));
  },
  _timeAgo(ms) {
    if (!ms) return '';
    const mins = Math.floor((Date.now() - ms) / 60000);
    if (mins < 1) return t('time.justNow');
    if (mins < 60) return this._fmt('time.minAgo', { n: mins });
    const hours = Math.floor(mins / 60);
    if (hours < 24) return this._fmt('time.hoursAgo', { n: hours });
    const days = Math.floor(hours / 24);
    return days === 1 ? t('time.yesterday') : this._fmt('time.daysAgo', { n: days });
  },
  // Each notification stores a `kind` (+ params) and is rendered in the
  // current language, so switching languages re-translates old notifications too.
  _notifView(n) {
    const p = { provider: n.providerName || '', service: n.service || '', date: n.date || '', text: n.text || '' };
    switch (n.kind) {
      case 'welcome':  return { icon: 'bell',   title: t('ntf.welcome.t'), desc: t('ntf.welcome.d') };
      case 'booked':   return { icon: 'check',  title: t('ntf.booked.t'),  desc: this._fmt('ntf.booked.d', p) };
      case 'message':  return { icon: 'share',  title: this._fmt('ntf.message.t', p), desc: p.text };
      case 'progress': {
        const icons = { enroute: 'nav', arrived: 'pin', inprogress: 'wrench', completed: 'star' };
        return { icon: icons[n.stage] || 'clock', title: t('ntf.progress.' + n.stage + '.t'), desc: this._fmt('ntf.progress.' + n.stage + '.d', p) };
      }
      default:         return { icon: 'bell', title: n.title || '', desc: n.desc || '' };
    }
  },
  Notifications() {
    const row = (n) => {
      const v = this._notifView(n);
      return `<div class="act-row card" style="margin-bottom:10px" onclick="App.markNotifRead('${n.id}')">
        <div class="ac-ic" style="background:var(--orange-soft);color:var(--orange)">${icon(v.icon)}</div>
        <div style="flex:1;min-width:0">
          <div class="at">${v.title}</div>
          <div class="as">${v.desc}</div>
        </div>
        <div class="atime">${!n.read ? '<span class="dot-unread"></span>' : ''}${this._timeAgo(n.createdAt)}</div>
      </div>`;
    };
    const list = this.state.notifications;
    return `<div class="screen">
      ${TopBar(t('pf.notifications'))}
      ${list.length ? `<div class="section-head pad" style="justify-content:flex-end;margin-top:8px">
        <div class="link" onclick="App.markAllNotifRead()">${t('ntf.markAllRead')}</div>
      </div>
      <div class="pad">${list.map(row).join('')}</div>`
      : EmptyState('bell', t('ntf.emptyTitle'), t('ntf.emptyDesc'), '', '')}
    </div>`;
  },
