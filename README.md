...
js/data.js            Mock data — Phnom Penh providers, services, activity
...
- Saved locations persist to `localStorage`; everything else is in-memory mock data
...
- **Firestore** — `providers` (public catalog, seeded once via `seed.html`; each provider has a `reviews` subcollection — one review per signed-in user, and the provider's rating/count is recomputed from them) and `users/{uid}` (favorites field, plus `vehicles`/`bookings`/`notifications` subcollections, created automatically on first sign-in). Notifications are generated by real events — a confirmed booking, a dispatcher updating progress or replying in chat from `admin.html` — and stream to the app live
